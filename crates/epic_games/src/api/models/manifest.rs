use crate::api::{Guid, MANIFEST_MAGIC, models::utils::read};
    ...
    pub fn new(bytes: Vec<u8>) -> Result<Self> {
        ...
        let mut decompressed_cursor = if header.compressed() {
            ...
            if data.len() != header.data_size as usize {
                return Err(anyhow!("Decompressed size does not match expected size"));
            }

            let computed_sha1 = Sha1::digest(&data);

            if computed_sha1.as_slice() != header.data_sha1 {
                return Err(anyhow!("SHA1 checksum does not match"));
            }
...
impl ManifestHeader {
    pub fn new(cursor: &mut Cursor<Vec<u8>>) -> Result<Self> {
        ...
        if magic != MANIFEST_MAGIC {
            return Err(anyhow!("Invalid manifest magic"));
        }
