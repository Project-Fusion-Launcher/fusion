...
fn hex_to_guid(hex_guid: &str) -> Result<Guid> {
    let bytes = const_hex::decode(hex_guid)?;

    if bytes.len() != 16 {
        return Err(anyhow!("GUID must be 16 bytes (32 hex chars) long"));
    }
...
fn hex_to_sha1(hex_str: &str) -> Result<[u8; 20]> {
    ...
    if bytes.len() != 20 {
        return Err(anyhow!("SHA1 hex string must be 40 characters long"));
    }
    let arr: [u8; 20] = bytes
        .try_into()
